...
NoteListDelegate::NoteListDelegate(NoteListView *view, TagPool *tagPool, QObject *parent)
    : QStyledItemDelegate(parent),
...
#ifdef __APPLE__
      m_titleFont(m_displayFont, 13, QFont::Bold),
      m_titleSelectedFont(m_displayFont, 13, QFont::Bold),
      m_contentFont(m_displayFont, 13, QFont::Normal),
      m_dateFont(m_displayFont, 11),
      ...
      m_applicationInactiveColor(175, 212, 228),
                                       ...
                                       const QModelIndex &index) const
{
    ...
    bufferRect.adjust(leftRightPadding,
                      dynamic_cast<NoteListView *>(m_view)->isDragging() ? 0 : topBottomPadding,
                      -(leftRightPadding + 1),
                      dynamic_cast<NoteListView *>(m_view)->isDragging() ? 0
                                                                         : -(topBottomPadding - 1));
    QPainterPath path;
    path.addRoundedRect(bufferRect, cornerRadius, cornerRadius); // Create rounded rectangle
    ...
    if (model && model->hasPinnedNote() && model->isFirstPinnedNote(index)
        ...
        bufferPainter.fillPath(path, QBrush(m_defaultColor));
        ...
        if (dynamic_cast<NoteListView *>(m_view)->isDragging()) {
            if (isPinned) {
                ...
                //                bufferPainter.fillPath(path, QBrush("#d6d5d5"));
            }
        } else {
            //            bufferPainter.fillRect(bufferRect, QBrush(m_hoverColor));
            bufferPainter.fillPath(path, QBrush(m_hoverColor));
        }
    } else {
        if (m_view->isPinnedNotesCollapsed()) {
            if (!isPinned) {
                                   ...
                                   const QModelIndex &index) const
{
    if (m_animatedIndexes.contains(index)) {
        ...
        //                = rowPosY + fmRectTitle.height() + fmRectDate.height()
        ...
        double dateRectPosY = rowPosY + fmRectTitle.height() + fmRectContent.height()
                + NoteListConstant::topOffsetY * 2.0 + yOffsets;
        //                rowPosY + fmRectTitle.height() + NoteListConstant::topOffsetY + yOffsets;
        ...
        if (m_isInAllNotes) {
            ...
            folderNameRectPosY = rowPosY + fmRectContent.height() + fmRectTitle.height()
                    + fmRectDate.height() + NoteListConstant::topOffsetY * 1.9 + yOffsets;
        ...
        if (m_isInAllNotes) {
            ...
            drawStr(folderNameRectPosX, folderNameRectPosY, folderNameRectWidth,
                    folderNameRectHeight, m_contentColor, m_contentFont, parentName);
        }
        drawStr(contentRectPosX, contentRectPosY, contentRectWidth, contentRectHeight,
                m_contentColor, m_contentFont, content);
        painter->setRenderHints(QPainter::Antialiasing | QPainter::SmoothPixmapTransform);
        int rowHeight;
        if (m_animatedIndexes.contains(index)) {
            if (m_state != NoteListState::MoveIn) {
        ...
        if (model && model->hasPinnedNote()) {
            if (model->isFirstPinnedNote(index)) {
                ...
                painter->setPen(m_theme == Theme::Dark ? QColor(162, 163, 164)
                                                       : QColor(105, 105, 105));
                if (m_view->isPinnedNotesCollapsed()) {
                    painter->drawText(QRect(headerRect.right() - 31, headerRect.y() + 5, 16, 16),
                                      u8"\uf054"); // fa-chevron-right
                } else {
                    painter->drawText(QRect(headerRect.right() - 31, headerRect.y() + 5, 16, 16),
                                      u8"\uf078"); // fa-chevron-down
        ...
        double rowPosY = option.rect.y() + 1;
        auto model = dynamic_cast<NoteListModel *>(m_view->model());
        int fifthYOffset = 0;
        if (model && model->hasPinnedNote() && !m_view->isPinnedNotesCollapsed()
            && model->isFirstUnpinnedNote(index)) {
...
void NoteListDelegate::paintSeparator(QPainter *painter, QRect rect, const QModelIndex &index) const
{
    ...
    int posX1 = rect.x() + leftOffsetX - 7;
    int posX2 = rect.x() + rect.width() - leftOffsetX / 2 + 10;
                                    ...
                                    const QModelIndex &index) const
{
    ...
    for (const auto &id : qAsConst(tagIds)) {
        ...
        QFontMetrics fmName(m_contentFont);
...
void NoteListDelegate::setTheme(Theme::Value theme)
{
    ...
    switch (m_theme) {
    case Theme::Light: {
        m_titleColor = QColor(68, 68, 68);
        m_dateColor = QColor(32, 36, 40);
        m_contentColor = QColor(128, 128, 128);
        ...
        m_applicationInactiveColor = QColor(175, 212, 228);
        m_separatorColor = QColor(219, 220, 221);
        break;
    }
    case Theme::Dark: {
        m_titleColor = QColor(255, 255, 255);
        m_dateColor = QColor(255, 255, 255, 127);
        ...
        m_separatorColor = QColor(54, 55, 57);
        break;
    }
    case Theme::Sepia: {
        m_titleColor = QColor(68, 68, 68);
        m_dateColor = QColor(32, 36, 40);
        m_contentColor = QColor(77, 77, 77);
