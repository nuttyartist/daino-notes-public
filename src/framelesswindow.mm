...
void CFramelessWindow::initUI()
{
    ...
    // m_closeButtonOriginX = closeButton.frame.origin.x;
    // m_minimizeButtonOriginX = minimizeButton.frame.origin.x;
    // m_zoomButtonOriginX = zoomButton.frame.origin.x;
    // m_closeButtonOriginY = closeButton.frame.origin.y;
    // m_minimizeButtonOriginY = minimizeButton.frame.origin.y;
    // m_zoomButtonOriginY = zoomButton.frame.origin.y;

    // closeButton.frame = NSMakeRect(closeButton.frame.origin.x + 5, closeButton.frame.origin.y - 1,
    //                                closeButton.frame.size.width, closeButton.frame.size.height);
    // minimizeButton.frame =
    //         NSMakeRect(minimizeButton.frame.origin.x + 5, minimizeButton.frame.origin.y - 1,
    //                    minimizeButton.frame.size.width, minimizeButton.frame.size.height);
    // zoomButton.frame = NSMakeRect(zoomButton.frame.origin.x + 5, zoomButton.frame.origin.y - 1,
...
void CFramelessWindow::setStandardWindowButtonsMacVisibility(bool isVisible)
{
    ...
    NSView *titleBarContainerView = closeButton.superview.superview;

    [titleBarContainerView setHidden:NO];
    CGFloat buttonHeight = [closeButton frame].size.height;
    CGFloat titleBarFrameHeight = buttonHeight + yOffset;
    ...
    NSArray *windowButtons = @[ closeButton, minimizeButton, zoomButton ];
    const CGFloat space_between = [minimizeButton frame].origin.x - [closeButton frame].origin.x;
    for (NSUInteger i = 0; i < windowButtons.count; i++) {
        NSView *viewButtons = [windowButtons objectAtIndex:i];
        CGRect rect = [viewButtons frame];
        rect.origin.x = xOffset + (i * space_between);
        rect.origin.y = (titleBarFrameHeight - rect.size.height) / 2;
        [viewButtons setFrameOrigin:rect.origin];
    }
}
