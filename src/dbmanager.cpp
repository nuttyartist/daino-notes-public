...
void DBManager::exportNotes(const QString &baseExportPath, const QString &extension)
{
    ...
    QString rootFolderName = QStringLiteral("Daino Notes");
...
int DBManager::getTotalNumberOfNotesNotInTrash()
{
    QSqlQuery query(m_db);
    query.prepare(R"(SELECT COUNT(*) FROM node_table WHERE node_type = :note_type AND parent_id != :trash_id)");
    query.bindValue(":note_type", static_cast<int>(NodeData::Type::Note));
    query.bindValue(":trash_id", SpecialNodeID::TrashFolder);

    if (!query.exec()) {
        qDebug() << "Failed to retrieve total number of notes:" << query.lastError();
        return -1;
    }
...
int DBManager::getTotalNumberOfFolders()
{
    QSqlQuery query(m_db);
    query.prepare(R"(SELECT COUNT(*) FROM node_table WHERE node_type = :folder_type)");
    query.bindValue(":folder_type", static_cast<int>(NodeData::Type::Folder));

    if (!query.exec()) {
        qDebug() << "Failed to retrieve total number of folders:" << query.lastError();
        return -1;
    }
...
int DBManager::getTotalNumberOfTags()
{
    QSqlQuery query(m_db);
    query.prepare(R"(SELECT COUNT(*) FROM tag_table)");

    if (!query.exec()) {
        qDebug() << "Failed to retrieve total number of tags:" << query.lastError();
        return -1;
    }
