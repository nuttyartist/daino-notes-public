...
void NoteListView::startDrag(Qt::DropActions supportedActions)
{
    ...
    if (indexes.size() == 1) {
...
#ifdef __APPLE__
        QFont m_displayFont(QFont(QFontInfo(QApplication::font()).family()).exactMatch()
        ...
        QFont m_displayFont(QFont(QStringLiteral("Segoe UI")).exactMatch()
                                    ...
                                    : QStringLiteral("Inter"));
#else
        QFont m_displayFont(QStringLiteral("Inter"));
#endif
    ...
    if (index.isValid()) {
        ...
        if (notes.size() > 1) {
            if (m_isInTrash) {
                ...
                deleteNoteAction->setText(tr("Move to Trash"));
            }
        } else {
            if (m_isInTrash) {
