...
TreeViewLogic::TreeViewLogic(NodeTreeView *treeView, NodeTreeModel *treeModel, DBManager *dbManager,
    ...
    : QObject(parent),
      ...
      m_lastSelectFolder{ QStringLiteral("") },
      m_lastSelectTags{},
      m_expandedFolder{}
{
...
void TreeViewLogic::loadTreeModel(const NodeTagTreeData &treeData)
{
    ...
    if (m_needLoadSavedState) {
        ...
        if (m_isLastSelectFolder) {
            ...
            } else if (m_lastSelectFolder.isEmpty()) {
                index = m_treeModel->getDefaultNotesIndex();
            } else {
        ...
        m_treeView->setCurrentIndexC(m_treeModel->getDefaultNotesIndex());
    }
