...
*/
DeclarativeMimeData::DeclarativeMimeData(const QMimeData* copy, QObject *source)
    : QMimeData(),
    m_source(0),
    m_image(QImage())
{
    ...
    // Because for some reason setData doesn't set the imageData or doesn't do it correctly
    m_image = qvariant_cast<QImage>(copy->imageData());
    emit imageChanged();
    // if (source != nullptr)
    //     setSource(qobject_cast<QQuickItem*>(source));
...
{
    QList<QUrl> urlList;
    Q_FOREACH (const QJsonValue &varUrl, urls) {
...
QImage DeclarativeMimeData::image() const
{
    // if (this->hasImage()) {
    //     return qvariant_cast<QImage>(this->imageData());
    // }
    // return QImage();
    return m_image;
}

void DeclarativeMimeData::setImage(const QVariant &image)
{
    QImage newImage = qvariant_cast<QImage>(image);

    if (m_image != newImage) {
        m_image = newImage; // Update the image
        emit imageChanged(); // Emit the signal to indicate the image has changed
    }
}
