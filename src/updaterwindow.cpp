...
static const QString
        UPDATES_URL("https://raw.githubusercontent.com/nuttyartist/daino-notes-public/main/UPDATES.json");
      ...
      m_manager(new QNetworkAccessManager(this))
{
    ...
    QFont fontToUse = QFont(QStringLiteral("Segoe UI")).exactMatch() ? QStringLiteral("Segoe UI")
                                                                     : QStringLiteral("Inter");
#else
    QFont fontToUse = QFont(QStringLiteral("Inter"));
#endif
...
#  else
    ...
    arguments << "/SILENT"; // Add the /SILENT parameter
    QProcess *process = new QProcess(this);
    bool isStarted = process->startDetached(file, arguments);
    if (isStarted) {
        qApp->quit();
    } else {
        bool openUrl = QDesktopServices::openUrl(QUrl::fromLocalFile(file));
