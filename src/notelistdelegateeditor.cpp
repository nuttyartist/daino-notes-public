      ...
      m_containsMouse(false)
{
    ...
    if (m_delegate->isInAllNotes()) {
        ...
        y += yOffsets;
        m_tagListView->setGeometry(10, y - 5, rect().width() - 24, m_tagListView->height());
    } else {
        int y = 70;
        auto model = dynamic_cast<NoteListModel *>(m_view->model());
        if (model) {
                                             ...
                                             const QModelIndex &index) const
{
    ...
    bufferRect.adjust(leftRightPadding, m_view->isDragging() ? 0 : topBottomPadding,
                      -(leftRightPadding + 1), m_view->isDragging() ? 0 : -(topBottomPadding - 1));
    auto model = dynamic_cast<NoteListModel *>(m_view->model());
    ...
    if (m_view->selectionModel()->isSelected(index)) {
        if (qApp->applicationState() == Qt::ApplicationActive) {
            if (m_isActive) {
                ...
                bufferPainter.fillPath(path, QBrush(m_ActiveColor));
                m_tagListView->setBackground(m_ActiveColor);
            } else {
                //                bufferPainter.fillRect(bufferRect, QBrush(m_notActiveColor));
                bufferPainter.fillPath(path, QBrush(m_notActiveColor));
            ...
            bufferPainter.fillPath(path, QBrush(m_applicationInactiveColor));
                                         ...
                                         const QModelIndex &index) const
{
    ...
    if (model && model->hasPinnedNote()) {
        if (model->isFirstPinnedNote(index)) {
            ...
            painter->setPen(m_theme == Theme::Dark ? QColor(162, 163, 164) : QColor(105, 105, 105));
    ...
    if (m_delegate->isInAllNotes()) {
        ...
        drawStr(folderNameRectPosX, folderNameRectPosY, folderNameRectWidth, folderNameRectHeight,
                m_contentColor, m_contentFont, parentName);
    }
                                            ...
                                            const QModelIndex &index) const
{
    ...
    int posX2 = rect().x() + rect().width() - leftOffsetX / 2 - 1;
...
void NoteListDelegateEditor::resizeEvent(QResizeEvent *event)
{
    ...
    if (m_delegate->isInAllNotes()) {
        ...
        m_tagListView->setGeometry(NoteListConstant::leftOffsetX - 5, y + 10, rect().width() - 42,
        ...
        m_tagListView->setGeometry(NoteListConstant::leftOffsetX - 5, y + 1, rect().width() - 42,
                                   m_tagListView->height());
    }
    recalculateSize();
}
